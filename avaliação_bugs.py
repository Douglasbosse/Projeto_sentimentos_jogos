...
df_bug = df[df["review"].str.contains(r"bug|bugs", case=False, na=False)]


avaliacao_positiva_bug = df_bug[df_bug["avaliacao"] == "Positiva"].shape[0]
avaliacao_negativa_bug = df_bug[df_bug["avaliacao"] == "Negativa"].shape[0]


total_comentarios_analisados = len(df)
total_comentarios_com_bug = len(df_bug)


labels = ['Positivas', 'Negativas']
sizes = [avaliacao_positiva_bug, avaliacao_negativa_bug]
...
plt.title(f"Avaliações para o Jogo: {nome_jogo}\n"
          ...
          f"Total de Comentários com 'bug' ou 'bugs': {total_comentarios_com_bug}")
plt.axis('equal') 
plt.show()


df_bug.to_csv("comentarios_bug.csv", index=False, encoding="utf-8", sep=';')  
print("Comentários com a palavra 'bug' ou 'bugs' exportados para 'comentarios_bug.csv'")
